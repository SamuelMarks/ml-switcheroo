...
class NvidiaSassHtmlParser(HTMLParser):
  """State-machine based HTML parser for extracting NVIDIA_SASS Instruction tables."""
  ...
  def handle_endtag(self, tag: str) -> None:
    ...
    NVIDIA_SASS instruction opcodes and descriptions upon closing 'tr' tags.
...
class NvidiaSassSpecImporter:
  """Facade for importing, parsing, and normalizing NVIDIA_SASS specifications from HTML files."""
  ...
  def parse_file(self, html_path: Path) -> typing.Dict[str, dict]:
    ...
    Args:
        html_path: Path to the .html file containing NVIDIA_SASS documentation.

    Returns:
        A dictionary mapping abstract operation names or TitleCase mnemonics to
        their corresponding NVIDIA_SASS instruction representation (with 'api' keys).
    """
    if not html_path.exists():
      log_error(f"File not found: {html_path}")
      return {}

    log_info(f"Parsing NVIDIA_SASS Spec: {html_path.name}...")
    ...
    for opcode, desc in parser.extracted_ops:
      ...
      # If the mnemonic exists but no mapping, we assume it's a specific NVIDIA_SASS op
    ...
    log_success(f"Extracted {len(mappings)} NVIDIA_SASS instructions.")
  ...
  def _infer_abstract_op(self, opcode: str, desc: str) -> Optional[str]:
    """Derive the Abstract Operation ID (e.g., 'Add') from the NVIDIA_SASS text.

    Uses mnemonic rules and substring/pattern analysis of the instruction description
    to map raw NVIDIA_SASS operations to standard ML-Switcheroo abstract operations.

    Args:
        opcode: The raw NVIDIA_SASS opcode string (e.g., 'FADD', 'FMUL').
