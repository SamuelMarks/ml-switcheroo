...
def transform_batch_norm(node: cst.Call, ctx: HookContext) -> cst.CSTNode:
  ...
  if not any(a.keyword and a.keyword.value == "use_running_average" for a in new_args):  # pragma: no branch
    # Ensure previous arg has comma
    if new_args and new_args[-1].comma == cst.MaybeSentinel.DEFAULT:  # pragma: no branch
  ...
  if not any(a.keyword and a.keyword.value == "mutable" for a in new_args):  # pragma: no branch
    # Ensure previous arg has comma
    if new_args and new_args[-1].comma == cst.MaybeSentinel.DEFAULT:  # pragma: no branch
      new_args[-1] = new_args[-1].with_changes(comma=cst.Comma(whitespace_after=cst.SimpleWhitespace(" ")))
