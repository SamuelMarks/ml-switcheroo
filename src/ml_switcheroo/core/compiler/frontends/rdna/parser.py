...
class RdnaParser:
  ...
  def _parse_operand(self) -> Operand:
    ...
    if token.kind == TokenType.IMMEDIATE:
      ...
      val = float(val_str) if "." in val_str and not is_hex else int(val_str, 16 if is_hex else 10)  # type: ignore
      return Immediate(value=val, is_hex=is_hex)  # type: ignore

    if token.kind == TokenType.LBRACKET:
      self._consume()
      base_reg = self._parse_operand()

      offset = 0
      next_tok = self._peek()
      if next_tok and next_tok.kind in (TokenType.PLUS, TokenType.MINUS):
        sign = 1 if next_tok.kind == TokenType.PLUS else -1
        self._consume()
        imm_tok = self._consume()
        if imm_tok.kind != TokenType.IMMEDIATE:
          raise SyntaxError("Expected immediate after +/-")
        val_str = imm_tok.value
        is_hex = "0x" in val_str.lower()
        offset = sign * int(val_str, 16 if is_hex else 10)

      rb = self._consume()
      if rb.kind != TokenType.RBRACKET:
        raise SyntaxError("Expected ]")

      return Memory(base=base_reg, offset=offset)  # type: ignore
  ...
  def _parse_register_range(self) -> Union[SGPR, VGPR]:
    ...
    return reg_class(index=start_idx, count=count)  # type: ignore
