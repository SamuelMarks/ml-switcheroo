...
class RdnaLexer:
  ...
  PATTERNS: List[Tuple[TokenType, str]] = [
    ...
    # 5. Modifiers (Common memory modifiers)
    # Note: These could be identifiers, but we match specific keywords for stricter parsing if needed
    (TokenType.MODIFIER, r"\b(glc|slc|dlc|off)\b"),
    # 6. Immediates
    # Hex
    ...
    while pos < length:
      ...
      for kind, regex in self.regex_pairs:
        ...
        if match:
          ...
          # We usually want the token to reflect the label syntax including colon
          # to distinguish it from a reference easily. The pattern does this.
