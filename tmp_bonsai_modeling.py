...
class Qwen3VLVisionConfig:
  ...
  def qwen3vl_2b(cls, use_fsdp: bool = False, use_tp: bool = False):
    shd = VisionShardingConfig.default(use_fsdp, use_tp) if (use_fsdp or use_tp) else VisionShardingConfig.no_sharding()
    return cls(
      depth=24,
      hidden_size=1024,
...
def init_cache(
  ...
  return [
    LayerCache(config.text_config, batch_size, cache_size, dtype) for _ in range(config.text_config.num_hidden_layers)
  ]
