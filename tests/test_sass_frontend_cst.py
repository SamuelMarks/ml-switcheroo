...
def test_sass_register() -> None:
  ...
  reg_neg: SassRegister = SassRegister(name="R1", negated=True)
  assert reg_neg.to_text() == "-R1"

  reg_abs: SassRegister = SassRegister(name="R2", absolute=True)
  assert reg_abs.to_text() == "|R2|"

  reg_neg_abs: SassRegister = SassRegister(name="R3", negated=True, absolute=True)
  assert reg_neg_abs.to_text() == "-|R3|"

  reg_trivia: SassRegister = SassRegister(name="R0")
...
def test_sass_predicate() -> None:
  ...
  pred_neg: SassPredicate = SassPredicate(name="P1", negated=True)
  assert pred_neg.to_text() == "!P1"

  pred_guard: SassPredicate = SassPredicate(name="P2", is_guard=True)
  assert pred_guard.to_text() == "@P2"

  pred_all: SassPredicate = SassPredicate(name="P3", negated=True, is_guard=True)
  assert pred_all.to_text() == "@!P3"

  pred_trivia: SassPredicate = SassPredicate(name="P0")
...
def test_sass_immediate() -> None:
  ...
  imm_hex: SassImmediate = SassImmediate(value=15, is_hex=True)
  assert imm_hex.to_text() == "0xf"

  imm_trivia: SassImmediate = SassImmediate(value=42)
...
def test_sass_memory() -> None:
  ...
  mem_c_bank_no_offset: SassMemory = SassMemory(base="c[0x0]")
  assert mem_c_bank_no_offset.to_text() == "c[0x0][0x0]"

  # Register based
  reg: SassRegister = SassRegister(name="R5")
  mem_reg_offset: SassMemory = SassMemory(base=reg, offset=16)
  assert mem_reg_offset.to_text() == "[R5 + 0x10]"

  mem_reg_no_offset: SassMemory = SassMemory(base=reg)
  assert mem_reg_no_offset.to_text() == "[R5]"

  mem_trivia: SassMemory = SassMemory(base=reg)
...
def test_sass_instruction() -> None:
  ...
  # With predicate
  pred: SassPredicate = SassPredicate(name="P0", is_guard=True)
  inst2: SassInstruction = SassInstruction(opcode="MOV", operands=[SassRegister(name="R0")], predicate=pred)
  assert inst2.to_text() == "@P0 MOV R0;"

  # Test trivia logic
  pred.trailing_trivia = [Trivia(" ")]
  inst3: SassInstruction = SassInstruction(opcode="MOV", operands=[SassRegister(name="R0")], predicate=pred)
  ...
  op1: SassRegister = SassRegister(name="R0")
  op2: SassRegister = SassRegister(name="R1")
  op2.leading_trivia = [Trivia(" ")]
  inst4: SassInstruction = SassInstruction(opcode="ADD", operands=[op1, op2])
...
def test_sass_directive() -> None:
  ...
  dir2: SassDirective = SassDirective(name="reqntid", params=["128", "1", "1"])
