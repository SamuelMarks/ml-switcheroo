"""Test suite for the Plugin Loss Wrapper module."""

from unittest.mock import MagicMock

import libcst as cst

from ml_switcheroo.plugins.loss_wrapper import transform_loss_reduction


def get_dummy_ctx(target_fw: str = "torch", current_op_id: str = "dummy", sharding_supported: bool = False) -> MagicMock:
  """Gets dummy ctx."""
  ctx: MagicMock = MagicMock()
  ctx.target_fw = target_fw
  ctx.current_op_id = current_op_id
  op_def: MagicMock = MagicMock()
  op_def.sharding_supported = sharding_supported
  ctx.semantics.get_operation.return_value = op_def
  return ctx


def test_plugin_loss_wrapper() -> None:
  """Verifies the behavior of plugin loss wrapper."""
  ctx: MagicMock = get_dummy_ctx(target_fw="unknown")
  ctx.semantics.get_operation.return_value = MagicMock(is_loss=True)
  ctx.lookup_api.return_value = "dummy.loss"
  node: cst.Call = cst.Call(
    func=cst.Name("dummy"), args=[cst.Arg(value=cst.Name("my_mode"), keyword=cst.Name("reduction"))]
  )
  transform_loss_reduction(node, ctx)
  ctx.current_op_id = ""
  ctx.lookup_api.return_value = "dummy.loss"
  transform_loss_reduction(node, ctx)
  ctx.current_op_id = "CrossEntropyLoss"
  ctx.lookup_api.side_effect = lambda x: "dummy.loss" if x == "CrossEntropyLoss" else None
  node2: cst.Call = cst.Call(
    func=cst.Name("dummy"), args=[cst.Arg(value=cst.SimpleString('"sum"'), keyword=cst.Name("reduction"))]
  )
  transform_loss_reduction(node2, ctx)
