...
def test_get_func_name() -> None:
  """Docstring."""
  node1: cst.Call = cst.Call(func=cst.Name("load"), args=[])
  assert _get_func_name(node1) == "load"

  node2: cst.Call = cst.Call(func=cst.Attribute(value=cst.Name("torch"), attr=cst.Name("save")), args=[])
  assert _get_func_name(node2) == "save"

  node3: cst.Call = cst.Call(func=cst.Call(func=cst.Name("other"), args=[]), args=[])
  assert _get_func_name(node3) is None


def test_get_arg() -> None:
  """Docstring."""
  arg_pos: cst.Arg = cst.Arg(value=cst.Name("x"))
  arg_kw: cst.Arg = cst.Arg(keyword=cst.Name("f"), value=cst.Name("y"))
  arg_kw2: cst.Arg = cst.Arg(keyword=cst.Name("obj"), value=cst.Name("z"))
  args: List[cst.Arg] = [arg_pos, arg_kw, arg_kw2]
...
def test_transform_io_calls(mock_capture: MagicMock, mock_get_adapter: MagicMock) -> None:
  """Docstring."""
  ...
  node: cst.Call = cst.Call(func=cst.Name("load"), args=[])
  ...
  # 2. No func name
  node2: cst.Call = cst.Call(func=cst.Call(func=cst.Name("f"), args=[]), args=[])
  assert transform_io_calls(node2, ctx) == node2

  # 3. Not save or load
  node3: cst.Call = cst.Call(func=cst.Name("other_func"), args=[])
  assert transform_io_calls(node3, ctx) == node3

  # 4. Save missing args
  node_save_missing: cst.Call = cst.Call(func=cst.Name("save"), args=[cst.Arg(value=cst.Name("x"))])
  assert transform_io_calls(node_save_missing, ctx) == node_save_missing

  # 5. Load missing args
  node_load_missing: cst.Call = cst.Call(func=cst.Name("load"), args=[])
  assert transform_io_calls(node_load_missing, ctx) == node_load_missing

  # 6. Save valid
  node_save: cst.Call = cst.Call(
  ...
  result: cst.CSTNode = transform_io_calls(node_save, ctx)
  ...
  assert getattr(result.func.attr, "value", None) == "save"  # "jax.save" is parsed

  # 7. Load valid
  node_load: cst.Call = cst.Call(func=cst.Name("load"), args=[cst.Arg(value=cst.Name("file_var"))])
  adapter.get_serialization_syntax.return_value = "jax.load(file_var)"
  result2: cst.CSTNode = transform_io_calls(node_load, ctx)
