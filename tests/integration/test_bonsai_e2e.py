...
QWEN_VL_SOURCE: str = "import torch\nimport torch.nn as nn\n\nclass VisionFrontEnd(nn.Module):\n    def __init__(self):\n        super().__init__()\n        self.patch_conv = nn.Conv3d(\n            in_channels=3,\n            out_channels=1280,\n            kernel_size=(2, 14, 14),\n            stride=(2, 14, 14),\n            bias=False\n        )\n\n    def forward(self, x):\n        return self.patch_conv(x)\n"


def test_qwen_to_flax_nnx(semantics: SemanticsManager) -> None:
  ...
  result: ConversionResult = engine.run(QWEN_SOURCE)
...
def test_qwen_vl_to_mlx(semantics: SemanticsManager) -> None:
  ...
  engine = ASTEngine(semantics=semantics, config=config)
  result: ConversionResult = engine.run(QWEN_VL_SOURCE)
  assert result.success
  code: str = result.code
...
def test_qwen_to_keras(semantics: SemanticsManager) -> None:
