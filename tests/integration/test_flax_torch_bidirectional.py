...
def test_flax_nnx_to_torch_neural_ex0(semantics):
  ...
  try:
    ...
    pytest.fail(f"AST Mismatch or Invalid Syntax: {e}\n\nExpected:\n{torch_tier2_ex0}\n\nActual:\n{result.code}")
...
def test_torch_to_flax_nnx_neural_ex0(semantics):
  ...
  assert "nnx.Linear(10, 10, rngs=rngs)" in result.code
  # 5. Method rename
...
def test_torch_bidirectional_flax_nnx_neural_ex0(semantics):
  ...
  try:
    assert cmp_ast(ast.parse(as_torch.code), ast.parse(torch_tier2_ex0))
