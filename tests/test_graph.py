...
"""
  ...
  edge_sources: List[str] = [e.source for e in extractor.graph.edges]
  edge_targets: List[str] = [e.target for e in extractor.graph.edges]
...
def test_graph_extractor_external_function() -> None:
  """Docstring."""
  code: str = """
class GeneratedNet:
    def forward(self, x):
...
def test_graph_extractor_top_level_flow() -> None:
  """Docstring."""
  code: str = """
x = 1
y = F.relu(x)
