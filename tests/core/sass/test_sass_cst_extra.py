...
def test_sass_cst_render_edge():
  """Docstring."""
  from ml_switcheroo.core.compiler.frontends.sass.cst import SassRegister, SassMemory, SassPredicate, SassLabel
  from ml_switcheroo.core.cst.base import Trivia

  r = SassRegister(name="R0", negated=True, absolute=True)
  r.leading_trivia = [Trivia(text=" ")]
  r.trailing_trivia = [Trivia(text=" ")]
  assert r.to_text() == " -|R0| "
...
def test_sass_instruction_cst():
  """Docstring."""
  from ml_switcheroo.core.compiler.frontends.sass.cst import SassInstruction, SassImmediate, SassPredicate
  import pytest

  with pytest.raises(ValueError):
    SassInstruction(opcode="invalid op")

  i = SassInstruction(opcode="NOP", operands=[SassImmediate(value=0), SassImmediate(value=1)])
...
def test_sass_instruction_trivia():
  """Docstring."""
  from ml_switcheroo.core.compiler.frontends.sass.cst import SassInstruction, SassImmediate, SassModule
  from ml_switcheroo.core.cst.base import Trivia

  op = SassImmediate(value=0)
  op.leading_trivia = [Trivia(text=" /* trivia */ ")]
  i = SassInstruction(opcode="NOP", operands=[op])
  assert "/* trivia */" in i.to_text()

  mod = SassModule(statements=[i])
