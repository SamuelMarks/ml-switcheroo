...
def test_sass_macro_rnn() -> None:
  ...
  nodes: list[SassNode] = expand_rnn(allocator, node_id, metadata)
  assert len(nodes) >= 10
...
def test_sass_macro_lstm() -> None:
  ...
  nodes: list[SassNode] = expand_lstm(allocator, node_id, metadata)
  assert len(nodes) >= 10
...
def test_sass_macro_gru() -> None:
  ...
  nodes: list[SassNode] = expand_gru(allocator, node_id, metadata)
  assert len(nodes) >= 10
...
def test_sass_analyzer_rnn() -> None:
