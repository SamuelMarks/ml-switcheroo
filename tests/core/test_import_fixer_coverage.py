...
def test_attributes_mixin_missing() -> None:
  ...
  res: typing.Any = fixer.leave_Attribute(node_to_simplify, node_to_simplify)
  assert res is not node_to_simplify
  node_no_name = cst.Attribute(value=cst.Call(func=cst.Name("a")), attr=cst.Name("b"))
  res_no_name: typing.Any = fixer.leave_Attribute(node_no_name, node_no_name)
  assert res_no_name == node_no_name
  fixer._path_to_alias = None  # type: ignore
  node_valid = cst.Attribute(value=cst.Name("a"), attr=cst.Name("b"))
  res_valid: typing.Any = fixer.leave_Attribute(node_valid, node_valid)
  ...
  res_unsafe: typing.Any = fixer.leave_Attribute(node_unsafe, node_unsafe)
  ...
  res_safe: typing.Any = fixer.leave_Attribute(node_safe, node_safe)
  ...
  res_collapse: typing.Any = fixer.leave_Attribute(node_collapse, node_collapse)
  assert res_collapse is not node_collapse


def test_attributes_mixin_no_full_name() -> None:
...
def test_imports_mixin_alias() -> None:
...
def test_imports_mixin_107() -> None:
...
def test_base_import_fixer() -> None:
  ...
  fixer2 = BaseImportFixer(plan, source_fws=["jax"])  # type: ignore
...
def test_attributes_mixin_line_40() -> None:
