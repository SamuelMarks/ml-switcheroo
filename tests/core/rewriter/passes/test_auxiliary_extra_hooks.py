...
def test_auxiliary_visit_simplestatementline():
  """Test function."""
  ctx = setup_ctx()
  p = AuxiliaryTransformer(ctx)
  res = p.visit_SimpleStatementLine(cst.SimpleStatementLine(body=[cst.Pass()]))
  assert res is True
  assert p.context.current_stmt_errors == []
  assert p.context.current_stmt_warnings == []


def test_auxiliary_leave_decorator_rename_noncall3():
  """Test function."""
  ctx = setup_ctx()
  ctx.semantics.get_definition = MagicMock(return_value=("id", {"variants": {"torch": {"no_api": "missing"}}}))
  p = AuxiliaryTransformer(ctx)
...
def test_auxiliary_for_loop_static_hook_same(mock_get_hook):
  """Test function."""
  ctx = setup_ctx()
  p = AuxiliaryTransformer(ctx)
...
@patch("ml_switcheroo.core.rewriter.passes.auxiliary.get_hook")
def test_auxiliary_for_loop_hook_same(mock_get_hook):
  """Test function."""
  ctx = setup_ctx()
  p = AuxiliaryTransformer(ctx)
  loop = cst.For(target=cst.Name("i"), iter=cst.Name("range"), body=cst.IndentedBlock(body=[]))
...
@patch("ml_switcheroo.core.rewriter.passes.auxiliary.get_hook")
def test_auxiliary_for_loop_hook_none(mock_get_hook):
  """Test function."""
  ctx = setup_ctx()
  p = AuxiliaryTransformer(ctx)
  loop = cst.For(target=cst.Name("i"), iter=cst.Name("range"), body=cst.IndentedBlock(body=[]))
