...
def test_fusion_return() -> None:
  ...
  code: str = gen_code([op_call, op_return])
  assert "return _fn()" in code
  assert "=" not in code


def test_fusion_setattr() -> None:
  ...
  code: str = gen_code([op_create, op_set])
  assert "_self.layer = layer(_x)" in code
  assert "_res" not in code


def test_fusion_no_fuse_if_multicount() -> None:
...
def test_atom_inlining_getattr() -> None:
  ...
  code: str = gen_code([op_get, op_call, op_ret])
  assert "_self.conv(_x)" in code
  assert "_attr =" not in code


def test_atom_inlining_constant() -> None:
