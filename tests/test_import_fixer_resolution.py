...
def test_importreq_signature():
  ...
  req3 = ImportReq(module="torch", subcomponent="nn", alias="nn")
  assert req3.signature == "torch.nn"

  req4 = ImportReq(module="torch", subcomponent="nn", alias="my_nn")
  assert req4.signature == "torch.nn : my_nn"

  req5 = ImportReq(module="jax.numpy", alias="jnp")
  assert req5.signature == "jax.numpy : jnp"

  req6 = ImportReq(module="jax.numpy", alias="numpy")
  assert req6.signature == "jax.numpy"


def test_qual_name_scanner():
  """Test element."""
  tree = cst.parse_module("import jax.numpy\nx = jax.numpy.sum()")
  scanner = _QualNameScanner("jax.numpy")
  tree.visit(scanner)
  assert scanner.found is True

  tree2 = cst.parse_module("x = jnp.sum()")
  scanner2 = _QualNameScanner("jax.numpy")
  tree2.visit(scanner2)
  assert scanner2.found is False

  # visit_Name
  tree3 = cst.parse_module("jax_numpy_sum()")
  scanner3 = _QualNameScanner("jax_numpy_sum")
  ...
  scanner4 = _QualNameScanner("jax.numpy")
  scanner4.found = True
  ...
  # Bad Attribute visit
  scanner5 = _QualNameScanner("jax.numpy")
...
def test_import_resolver():
  """Test element."""
  sm = MockSemantics()
  resolver = ImportResolver(sm)  # type: ignore

  # Test 1: Framework Base Check
  tree1 = cst.parse_module("import jax\njax.sum()")
  plan = resolver.resolve(tree1, "jax")
  assert any(r.signature == "jax" for r in plan.required_imports)

  # Test 2: Framework Alias Check - by alias name
  tree2 = cst.parse_module("jnp.array([1, 2])")
  plan = resolver.resolve(tree2, "jax")
  assert any(r.signature == "jax.numpy : jnp" for r in plan.required_imports)
  assert plan.path_to_alias["jax.numpy"] == "jnp"

  # Test 2: Framework Alias Check - by mod path
  tree3 = cst.parse_module("jax.numpy.array([1, 2])")
  plan = resolver.resolve(tree3, "jax")
  assert any(r.signature == "jax.numpy : jnp" for r in plan.required_imports)

  # Test 3: Import Map Check - by alias name
  tree4 = cst.parse_module("nnx.Linear()")
  plan = resolver.resolve(tree4, "jax")
  assert any(r.signature == "flax.nnx" for r in plan.required_imports)
  assert plan.mappings["torch.nn"].module == "flax"
  assert plan.path_to_alias["flax.nnx"] == "nnx"

  # Test 3: Import Map Check - by full path
  tree5 = cst.parse_module("flax.nnx.Linear()")
  plan = resolver.resolve(tree5, "jax")
  assert any(r.signature == "flax.nnx" for r in plan.required_imports)
  ...
  sm2 = MockSemantics2()
  resolver2 = ImportResolver(sm2)  # type: ignore
  tree6 = cst.parse_module("optax.adam()")
  plan2 = resolver2.resolve(tree6, "jax")
  assert any(r.signature == "optax" for r in plan2.required_imports)

  # Check deduplicate
  reqs = [ImportReq("a"), ImportReq("a"), ImportReq("b")]
  dedup = _deduplicate(reqs)
  assert len(dedup) == 2
