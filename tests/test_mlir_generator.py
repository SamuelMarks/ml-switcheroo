...
def test_generate_module():
  ...
  # Empty module
  mod = ModuleNode(body=BlockNode(label="", operations=[]))
...
def test_convert_trivia():
  ...
  trivia_comment = Trivia(text="// a comment")
  trivia_percent = Trivia(text="% another comment")

  lines = gen._convert_trivia([trivia_comment, trivia_percent])
  assert len(lines) == 2
  assert lines[0].comment.value == "# a comment"
  assert lines[1].comment.value == "#% another comment"


def test_scan_block_usage():
  """Test element."""
  gen = MlirToPythonGenerator()

  # Op with operands
  op1 = OperationNode(name='"sw.op"', operands=[ValueNode(name="%0")])
  op2 = OperationNode(name='"sw.op"', operands=[ValueNode(name="%1"), ValueNode(name="%0")])

  mod = ModuleNode(body=BlockNode(label="", operations=[op1, op2]))
...
def test_should_inline_expression():
  """Test element."""
  gen = MlirToPythonGenerator()

  # no result op
  op_no_res = OperationNode(name='"sw.op"')
  assert not gen._should_inline_expression(op_no_res, cst.Name("x"))

  # sw.constant used
  op_const = OperationNode(name='"sw.constant"', results=[ValueNode(name="%0")])
  gen.usage_counts["%0"] = 1
  assert gen._should_inline_expression(op_const, cst.Name("cst"))

  # sw.getattr used
  op_getattr = OperationNode(name='"sw.getattr"', results=[ValueNode(name="%1")])
  ...
  op_normal = OperationNode(name='"sw.op"', results=[ValueNode(name="%3")])
  gen.usage_counts["%3"] = 1
  consumer = OperationNode(name='"sw.return"')
...
def test_resolve_operand():
  ...
  assert gen._resolve_operand("%0").value == "deferred_var"

  # Normal lookup
  gen.ctx.register("%1", hint="foo")
  assert gen._resolve_operand("%1").value == "_foo"

  # Dotted lookup
  gen.ctx.lookup = lambda x: "self.layer" if x == "%2" else "_foo"
  resolved = gen._resolve_operand("%2")
  assert isinstance(resolved, cst.Attribute)
  assert resolved.attr.value == "layer"
...
def test_create_expression_from_op():
  ...
  op_unknown = OperationNode(name='"unknown"')
  assert gen._create_expression_from_op(op_unknown) is None

  # We mock _expr_sw_constant
  op_const = OperationNode(name='"sw.constant"', attributes=[AttributeNode(name="value", value="42")])
  expr = gen._create_expression_from_op(op_const)
...
def test_convert_statement_op():
  ...
  op_return = OperationNode(name='"sw.return"')
  stmt = gen._convert_statement_op(op_return)
...
def test_wrap_as_statement():
  ...
  # No results
  op_no_res = OperationNode(name='"sw.op"')
  stmt = gen._wrap_as_statement(op_no_res, cst.Name("x"))
  assert isinstance(stmt, cst.SimpleStatementLine)
  assert isinstance(stmt.body[0], cst.Expr)

  # Usage count 0 -> Expr
  op_res = OperationNode(name='"sw.op"', results=[ValueNode(name="%0")])
  gen.usage_counts["%0"] = 0
  stmt = gen._wrap_as_statement(op_res, cst.Name("x"))
  assert isinstance(stmt.body[0], cst.Expr)

  # Void pattern
  void_call = cst.Call(func=cst.Attribute(value=cst.Call(func=cst.Name("super")), attr=cst.Name("__init__")))
  stmt = gen._wrap_as_statement(op_res, void_call)
  assert isinstance(stmt.body[0], cst.Expr)

  # Assignment with hint from 'type'
  op_type_hint = OperationNode(
  ...
  assert stmt.body[0].targets[0].target.value == "_flatten"

  # Assignment with hint from 'name'
  op_name_hint = OperationNode(
  ...
  assert stmt.body[0].targets[0].target.value == "_my_attr"

  # Assignment for sw.constant
  op_const_hint = OperationNode(name='"sw.constant"', results=[ValueNode(name="%3")])
  gen.usage_counts["%3"] = 1
  stmt = gen._wrap_as_statement(op_const_hint, cst.Name("x"))
  assert stmt.body[0].targets[0].target.value == "_cst"
...
def test_convert_block():
  ...
  block = BlockNode(label="", operations=[op_const, op_return])
  stmts = gen._convert_block(block)
  assert len(stmts) == 1
  assert isinstance(stmts[0].body[0], cst.Return)


def test_convert_block_deferred():
  """Test element."""
  gen = MlirToPythonGenerator()
  ...
  gen.usage_counts["%0"] = 1
  block = BlockNode(label="", operations=[op_const])
  stmts = gen._convert_block(block)
  assert len(stmts) == 0
  assert "%0" in gen.deferred_exprs


def test_scan_block_usage_regions():
  """Test element."""
  gen = MlirToPythonGenerator()
  b = BlockNode(label="", operations=[OperationNode(name='"sw.op"', operands=[ValueNode(name="%0")])])
  op = OperationNode(name='"sw.parent"', regions=[RegionNode(blocks=[b])])
  gen._analyze_module_usage(ModuleNode(body=BlockNode(label="", operations=[op])))
  assert gen.usage_counts["%0"] == 1


def test_convert_trivia_other():
  """Test element."""
  gen = MlirToPythonGenerator()
  lines = gen._convert_trivia([Trivia(text="other"), Trivia(text="// comment")])
  assert len(lines) == 1
  assert lines[0].comment.value == "# comment"


def test_convert_block_stmt_leading():
  """Test element."""
  gen = MlirToPythonGenerator()
  # To test stmt_node with leading trivia and no expression
  # We mock _convert_statement_op
  op1 = OperationNode(name='"sw.unknown"', leading_trivia=[Trivia(text="// a")])
  op2 = OperationNode(name='"sw.statement"', leading_trivia=[Trivia(text="// b")])
  ...
  with mock.patch.object(gen, "_create_expression_from_op", return_value=None):
    with mock.patch.object(
      ...
      stmts = gen._convert_block(BlockNode(label="", operations=[op1, op2]))
      assert len(stmts) == 1
      assert len(stmts[0].leading_lines) == 1
      assert stmts[0].leading_lines[0].comment.value == "# b"


def test_should_inline_expression_not_statement():
  """Test element."""
  gen = MlirToPythonGenerator()
  op_normal = OperationNode(name='"sw.op"', results=[ValueNode(name="%3")])
  gen.usage_counts["%3"] = 1
  consumer = OperationNode(name='"sw.other_consumer"')
  gen.usage_consumers["%3"] = consumer
  assert not gen._should_inline_expression(op_normal, cst.Name("norm"))


def test_create_expression_from_op_branches():
  """Test element."""
  gen = MlirToPythonGenerator()
...
def test_convert_statement_op_branches():
  """Test element."""
  gen = MlirToPythonGenerator()
...
def test_wrap_as_statement_branches():
  """Test element."""
  gen = MlirToPythonGenerator()
  # sw.getattr without raw_n
  op_getattr_no_n = OperationNode(name='"sw.getattr"', results=[ValueNode(name="%2")])
  gen.usage_counts["%2"] = 1
  stmt = gen._wrap_as_statement(op_getattr_no_n, cst.Name("x"))
  assert stmt.body[0].targets[0].target.value == "_2"  # or whatever auto-assigned


def test_is_void_call_super():
  """Test element."""
  gen = MlirToPythonGenerator()
  void_call = cst.Call(
  ...
  op_res = OperationNode(name='"sw.op"', results=[ValueNode(name="%9")])
