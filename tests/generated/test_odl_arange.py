...
def test_gen_Arange():
  # Generated Test for Arange
  # 1. Inputs
  np_start = random.randint(1, 3)
  np_stop = random.randint(1, 3)
  np_step = random.randint(1, 3)
  np_dtype = None
  results = {}
  ...
  if _FLAX_NNX_AVAILABLE:
    try:
      ...
      fn = lambda a0, a1, a2, a3: jax.numpy.arange(a0, a1, a2, a3)
      jitted_fn = jax.jit(fn, static_argnums=(3,))
      res = jitted_fn(np_start, np_stop, np_step, np_dtype)
  ...
  if _KERAS_AVAILABLE:
    try:
      res = keras.ops.arange(np_start, np_stop, np_step, np_dtype)
  ...
  if _MLX_AVAILABLE:
    try:
      res = mlx.core.arange(np_start, np_stop, np_step, np_dtype)
  ...
  if _NUMPY_AVAILABLE:
    try:
      res = numpy.arange(np_start, np_stop, np_step, np_dtype)
      results["numpy"] = res
    except Exception as e:
  ...
  if TENSORFLOW_AVAILABLE:
    try:
      res = tf.range(np_start, np_stop, np_step, np_dtype)
  ...
  if TORCH_AVAILABLE:
    try:
      res = torch.arange(np_start, np_stop, np_step, np_dtype)
