...
def test_gen_Array():
  # Generated Test for Array
  # 1. Inputs
  np_data = np.random.randn(2, 2, 2).astype(np.float32)
  np_dtype = None
  results = {}
  ...
  if JAX_AVAILABLE:
    try:
      ...
      fn = lambda a0, a1: jax.numpy.array(a0, a1)
      jitted_fn = jax.jit(fn, static_argnums=(1,))
      res = jitted_fn(np_data, np_dtype)
  ...
  if _KERAS_AVAILABLE:
    try:
      res = keras.ops.convert_to_tensor(np_data, np_dtype)
  ...
  if _MLX_AVAILABLE:
    try:
      res = mlx.core.array(np_data, np_dtype)
  ...
  if _NUMPY_AVAILABLE:
    try:
      res = numpy.array(np_data, np_dtype)
      results["numpy"] = res
    except Exception as e:
  ...
  if TENSORFLOW_AVAILABLE:
    try:
      res = tf.constant(np_data, np_dtype)
  ...
  if TORCH_AVAILABLE:
    try:
      res = torch.tensor(np_data, np_dtype)
