...
from ml_switcheroo.tools.injector_fw import _convert_to_cst_literal
...
def test_primitive_recursion():
  ...
  assert render_node(_convert_to_cst_literal(1)) == "1"
  assert render_node(_convert_to_cst_literal(1.5)) == "1.5"
  assert render_node(_convert_to_cst_literal(True)) == "True"
  assert render_node(_convert_to_cst_literal(None)) == "None"
  assert render_node(_convert_to_cst_literal("foo")) == '"foo"'


def test_list_recursion():
  # Simple list
  val = [1, 2, "a"]
  node = _convert_to_cst_literal(val)
