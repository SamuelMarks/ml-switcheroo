...
def test_filename_normalization(plugin_dir: Path) -> None:
  """Verifies the behavior of filename normalization."""
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="MyCustomHook", type=PluginType.CALL, doc="Test")
  gen.generate(scaffold)
  assert (plugin_dir / "my_custom_hook.py").exists()
  assert not (plugin_dir / "MyCustomHook.py").exists()
  scaffold2: PluginScaffoldDef = PluginScaffoldDef(name="tensorOps", type=PluginType.CALL, doc="Test")
  gen.generate(scaffold2)
  assert (plugin_dir / "tensor_ops.py").exists()
  scaffold3: PluginScaffoldDef = PluginScaffoldDef(name="already_valid", type=PluginType.CALL, doc="Test")
  gen.generate(scaffold3)
  assert (plugin_dir / "already_valid.py").exists()
  content: str = (plugin_dir / "my_custom_hook.py").read_text("utf-8")
...
def test_func_finder_other_func() -> None:
  ...
  finder: BodyExtractor = BodyExtractor("target_func")
  tree.visit(finder)
  assert getattr(finder, "found") is True


def test_generate_call_plugin(plugin_dir: Path) -> None:
  """Generates call plugin."""
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="my_hook", type=PluginType.CALL, doc="Test Hook")
  created: bool = gen.generate(scaffold)
  assert created is True
  file_path: Path = plugin_dir / "my_hook.py"
...
def test_generate_block_plugin(plugin_dir: Path) -> None:
  ...
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="loop_hook", type=PluginType.BLOCK, doc="Loop transform")
  created: bool = gen.generate(scaffold)
  assert created is True
  content: str = (plugin_dir / "loop_hook.py").read_text("utf-8")
...
def test_generate_creates_directory(tmp_path: Path) -> None:
  ...
  missing_dir: Path = tmp_path / "ghost_plugins"
  gen: PluginGenerator = PluginGenerator(missing_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="test", type=PluginType.CALL, doc="d")
  created: bool = gen.generate(scaffold)
  assert created is True
...
def test_generate_plugin_with_rules(plugin_dir: Path) -> None:
  ...
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="rule_hook", type=PluginType.CALL, doc="Rules", rules=rules)
  gen.generate(scaffold)
  file_path: Path = plugin_dir / "rule_hook.py"
  content: str = file_path.read_text("utf-8")
...
def test_preserves_user_logic(plugin_dir: Path) -> None:
  """Verifies the behavior of preserves user logic."""
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="custom_logic", type=PluginType.CALL, doc="Original Doc")
  gen.generate(scaffold)
  file_path: Path = plugin_dir / "custom_logic.py"
  ...
  new_scaffold: PluginScaffoldDef = PluginScaffoldDef(name="custom_logic", type=PluginType.CALL, doc="Updated Docstring")
...
def test_preserves_logic_with_complex_indentation(plugin_dir: Path) -> None:
  """Verifies the behavior of preserves logic with complex indentation."""
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="indent_test", type=PluginType.CALL, doc="Doc")
  gen.generate(scaffold)
  file_path: Path = plugin_dir / "indent_test.py"
...
def test_user_logic_trumps_rules(plugin_dir: Path) -> None:
  """Verifies the behavior of user logic trumps rules."""
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="priority_test", type=PluginType.CALL, doc="Doc")
  gen.generate(scaffold)
  file_path: Path = plugin_dir / "priority_test.py"
  file_path.write_text('\n@register_hook("priority_test")\ndef priority_test(node, ctx):\n    return "UserLogic"\n')
  rules: List[Rule] = [Rule(if_arg="x", is_val=1, use_api="y")]
  rule_scaffold: PluginScaffoldDef = PluginScaffoldDef(name="priority_test", type=PluginType.CALL, doc="Doc", rules=rules)
...
def test_overwrite_on_syntax_error(plugin_dir: Path, capsys: pytest.CaptureFixture[str]) -> None:
  """Verifies the behavior of overwrite on syntax correctly handling an error."""
  gen: PluginGenerator = PluginGenerator(plugin_dir)
  scaffold: PluginScaffoldDef = PluginScaffoldDef(name="broken_file", type=PluginType.CALL, doc="Doc")
  file_path: Path = plugin_dir / "broken_file.py"
...
def test_auto_wire_generation(plugin_dir: Path) -> None:
  ...
  auto_data: Dict[str, Any] = {
    "ops": {"TestOp": {"std_args": ["x"], "variants": {"jax": {"api": "foo", "requires_plugin": "rewired"}}}}
  }
  scaffold: PluginScaffoldDef = PluginScaffoldDef(
    name="rewired", type=PluginType.CALL, doc="Auto Wired", auto_wire=auto_data
  )
  gen.generate(scaffold)
  file_path: Path = plugin_dir / "rewired.py"
  content: str = file_path.read_text("utf-8")
