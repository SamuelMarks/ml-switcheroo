...
def test_tensor_type() -> None:
  ...
  t1: TensorType = TensorType(f32, [])
  assert t1.to_string() == "tensor<f32>"

  t2: TensorType = TensorType(f32, [2, "?", 4])
...
def test_function_type() -> None:
  ...
  f1: FunctionType = FunctionType([f32, i32], [])
  assert f1.to_string() == "(f32, i32) -> ()"

  f2: FunctionType = FunctionType([f32], [f32, i32])
  assert f2.to_string() == "(f32) -> (f32, i32)"

  f3: FunctionType = FunctionType([f32], [f32])
