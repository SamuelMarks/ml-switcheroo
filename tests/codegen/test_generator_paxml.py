...
def test_paxml_code_generation(tmp_path: Path) -> None:
  """Verifies the behavior of Paxml code generation."""
  semantics: dict[str, typing.Any] = {
    "abs": {"std_args": ["x"], "variants": {"paxml": {"api": "jnp.abs"}, "numpy": {"api": "np.abs"}}}
  }
  ...
  out_file: Path = tmp_path / "test_pax_generated.py"
  gen.generate(semantics, out_file)
  assert out_file.exists()
  content: str = out_file.read_text(encoding="utf-8")
  runtime_file: Path = out_file.parent / "runtime.py"
  assert runtime_file.exists()
  runtime_content: str = runtime_file.read_text(encoding="utf-8")
  assert 'find_spec("praxis")' in runtime_content
  assert 'find_spec("jax")' in runtime_content
  pax_block_start: int = content.find("Framework: paxml")
  assert pax_block_start != -1
  pax_block: str = content[pax_block_start:]
