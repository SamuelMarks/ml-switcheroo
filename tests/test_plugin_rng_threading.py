...
def test_remove_generator_arg():
  """Docstring."""
  # Has generator
  args = [cst.Arg(value=cst.Name("x")), cst.Arg(keyword=cst.Name("generator"), value=cst.Name("gen"))]
  cleaned = _remove_generator_arg(args)
  assert len(cleaned) == 1
  assert cleaned[0].value.value == "x"

  # No generator
  args = [cst.Arg(value=cst.Name("x")), cst.Arg(keyword=cst.Name("other"), value=cst.Name("gen"))]
  cleaned = _remove_generator_arg(args)
  assert len(cleaned) == 2


@patch("ml_switcheroo.plugins.rng_threading.get_adapter")
def test_inject_prng_threading_enabled(mock_get_adapter):
  """Docstring."""
  ...
  node = cst.parse_expression("dropout(x, generator=gen)")

  new_node = inject_prng_threading(node, ctx)
...
@patch("ml_switcheroo.plugins.rng_threading.get_adapter")
def test_inject_prng_threading_disabled(mock_get_adapter):
  """Docstring."""
  ctx = MagicMock()
  ctx.plugin_traits.requires_explicit_rng = False

  node = cst.parse_expression("dropout(x)")
...
@patch("ml_switcheroo.plugins.rng_threading.get_adapter")
def test_inject_prng_threading_no_adapter_or_split(mock_get_adapter):
  """Docstring."""
