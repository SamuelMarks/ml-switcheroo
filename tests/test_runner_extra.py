...
def test_runner_error_branches():
  """Test element."""
  ...
  with patch.object(runner, "_execute_api", side_effect=Exception("Mock Crash")):
    res, msg = runner.verify({"tf": {"api": "tf.add"}}, ["x"], None, None)
    assert res is False
    assert "Crash" in msg


def test_runner_hypothesis_exception():
  """Test element."""
  runner = EquivalenceRunner()
  ...
  res, msg = runner.verify({"tf": {"api": "tf.add"}, "torch": {"api": "torch.add"}}, ["x"], None, None)
  assert res is False
  assert "Verification Failed" in msg


def test_runner_misc_misses():
  """Test element."""
  runner = EquivalenceRunner()

  # 93 continue
  runner.verify({"bad_fw": "not_a_dict", "bad_fw2": {}}, [], None, None)
