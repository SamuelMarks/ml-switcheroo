...
def test_nvidia_sass_adapter_init() -> None:
  """Verifies the behavior of NVIDIA_SASS adapter initialization."""
...
def test_nvidia_sass_properties() -> None:
  """Verifies the behavior of NVIDIA_SASS properties."""
  adapter = NvidiaSassAdapter()
  assert adapter.import_alias == ("nvidia_sass", "asm")
  ...
  assert "// NVIDIA_SASS Header" in config["import"]
...
def test_nvidia_sass_missing_coverage() -> None:
  """Docstring."""
  adapter = NvidiaSassAdapter()
  ...
  assert adapter.get_weight_load_code("path") == "// Weights loading not supported in NVIDIA_SASS adapter"
  assert adapter.get_tensor_to_numpy_expr("my_var") == "my_var"
  assert adapter.get_weight_save_code("state", "path") == "// Weights saving not supported in NVIDIA_SASS adapter"
  ...
  L_LABEL:
  """
  graph_loop: typing.Any = adapter.parse_nvidia_sass_to_graph(code_loop)
  ...
  FFMA R1, R3, R5, R1
  """
  graph_no_loop: typing.Any = adapter.parse_nvidia_sass_to_graph(code_no_loop)
...
# --- Merged from test_nvidia_sass_extra.py ---


def test_nvidia_sass_missing_methods() -> None:
  """Docstring."""
  adapter = NvidiaSassAdapter()
...
def test_nvidia_sass_parse_nvidia_sass_to_graph() -> None:
  """Docstring."""
  adapter = NvidiaSassAdapter()

  # Test empty graph
  empty_graph: typing.Any = adapter.parse_nvidia_sass_to_graph("// just a comment")
  ...
  graph1: typing.Any = adapter.parse_nvidia_sass_to_graph(code)
  ...
  code_no_loop: str = """
    FFMA R1, R2, R3, R4
    """
  graph2: typing.Any = adapter.parse_nvidia_sass_to_graph(code_no_loop)
