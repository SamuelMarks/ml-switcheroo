...
def test_discovery_segmentation():
  ...
  with patch.dict(sys.modules, {"flax": MagicMock(), "flax.nnx": MagicMock()}):
    ...
    # Fix for CI: If flax.nnx was not importable at module level (top of file),
    # the adapter initializes in GHOST mode by default.
    # We explicitly force LIVE mode here to verify the search_modules string logic,
    # which is what we are testing here (not the import detection itself).
    flax_adapter._mode = InitMode.LIVE

    # Core should not look for NNX
    assert "flax.nnx" not in core.search_modules

    # Flax should look for both (or at least NNX)
    assert "flax.nnx" in flax_adapter.search_modules
    assert "jax.numpy" in flax_adapter.search_modules
