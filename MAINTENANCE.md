...
**Note:** The `sync` command writes discovered variants to `src/ml_switcheroo/snapshots/{fw}_mappings.json`. It combines findings from all Semantic Tiers (Math, Neural, Extras) into a single overlay file per framework. 
